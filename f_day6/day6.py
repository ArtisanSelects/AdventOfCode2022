...
def solve_puzzle(datastream, distinct_character_count):
    ...
    for i in range(len(datastream)-distinct_character_count-1):
        if len(set(datastream[i:i+distinct_character_count])) == distinct_character_count:
            return marker
        marker += 1
    raise Exception(f'Failed to find the marker with distinct character count of {distinct_character_count}.')
